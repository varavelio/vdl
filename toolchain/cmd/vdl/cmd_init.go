...
//go:embed cmd_init_config.vdl
var initConfig []byte

type cmdInitArgs struct {
	Path string `arg:"positional" help:"Directory where the VDL project will be initialized (default: current directory)"`
}
...
func cmdInit(args *cmdInitArgs) {
	...
	if info, err := os.Stat(args.Path); err == nil && !info.IsDir() {
		printFatal("VDL error: path must be a directory, not a file: %s", args.Path)
	}
	...
	printSuccess("VDL: files initialized:\n  - %s\n  - %s", schemaPath, configPath)
}
...
func generateUniqueFilenames(dir string) (string, string, string, string) {
	...
	configName = fmt.Sprintf("vdl.config-%d.vdl", timestamp)
