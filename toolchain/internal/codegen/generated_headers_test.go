...
func TestApplyGeneratedFileHeaders(t *testing.T) {
	t.Run("prepends header to supported files", func(t *testing.T) {
		...
		results := []executedPlugin{
			{
				Plugin: runtimePlugin{
					Source: pluginSource{
						Original:    "owner/vdl-plugin-demo@v1.0.0",
		...
		expectedHeader := buildGeneratedFileHeader(
			"owner/vdl-plugin-demo@v1.0.0 (hash 12345678)",
			lineSlashComment,
		)
		require.Equal(
			t,
			expectedHeader+"package main\n",
	...
	t.Run("uses matching comment syntax for sql files", func(t *testing.T) {
		...
		results := []executedPlugin{
			{
				Plugin: runtimePlugin{
					Source:         pluginSource{Original: "https://example.com/plugin.js"},
					GenerateHeader: true,
				},
				Output: plugintypes.PluginOutput{Files: &files},
			},
	...
	t.Run("uses block comments for html-like files", func(t *testing.T) {
		...
		results := []executedPlugin{
			{
				Plugin: runtimePlugin{
					Source:         pluginSource{Original: "./plugin.js"},
					GenerateHeader: true,
				},
				Output: plugintypes.PluginOutput{Files: &files},
			},
