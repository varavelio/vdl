...
VdlError _asError(Object err) {
  if (err is VdlError) return err;
  return VdlError(message: err.toString());
}
...
class _InternalClient {
  ...
  Future<Response<dynamic>> callProc(
    ...
    for (var attempt = 1; attempt <= retryConf.maxAttempts; attempt++) {
      try {
        ...
        try {
          ...
          return Response.error(VdlError(message: 'Invalid JSON response'));
        } catch (err) {
          return Response.error(_asError(err));
        }
      } on TimeoutException {
        final timeoutError = VdlError(
    ...
    return Response.error(lastError ?? VdlError(message: 'Unknown error'));
  }
              ...
              if (line.startsWith('data:')) {
                ...
                try {
                  ...
                  if (parsed is Map<String, dynamic>) {
                    ...
                    yield Response.error(
                      VdlError(message: 'Invalid event JSON'),
                    );
                    return;
                  }
                } catch (err) {
