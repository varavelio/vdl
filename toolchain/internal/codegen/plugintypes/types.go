...
// GetErrors returns the value of Errors or the zero value if the receiver or field is nil.
...
// GetErrorsOr returns the value of Errors or the provided default if the receiver or field is nil.
...
func (p *prePluginOutput) validate() error {
	...
	// Validation for field "errors"
	if p.Errors != nil {
		for _, item := range *p.Errors {
			if err := item.validate(); err != nil {
				return errorMissingRequiredField("field errors: " + err.Error())
			}
		}
	}
...
func (p *prePluginOutput) transform() PluginOutput {
	...
	if p.Errors != nil {
		var valErrors []PluginOutputError
		valErrors = make([]PluginOutputError, len(*p.Errors))
		for i, v := range *p.Errors {
			var tmp_ PluginOutputError
			tmp_ = v.transform()
			valErrors[i] = tmp_
		}
		transErrors = &valErrors
	}
...
// GetMessage returns the value of Message or the zero value if the receiver or field is nil.
...
// GetMessageOr returns the value of Message or the provided default if the receiver or field is nil.
...
// prePluginOutputError is the version of PluginOutputError previous to the required field validation
type prePluginOutputError struct {
	Message  *string      `json:"message,omitempty"`
	Position *prePosition `json:"position,omitempty"`
}

// validate validates the required fields of PluginOutputError
func (p *prePluginOutputError) validate() error {
	if p == nil {
		return errorMissingRequiredField("prePluginOutputError is nil")
	}

	// Validation for field "message"
	if p.Message == nil {
		return errorMissingRequiredField("field message is required")
	}
...
// transform transforms the prePluginOutputError type to the final PluginOutputError type
func (p *prePluginOutputError) transform() PluginOutputError {
	// Transformations
	transMessage := *p.Message
	var transPosition *Position
	if p.Position != nil {
		var valPosition Position
		valPosition = p.Position.transform()
		transPosition = &valPosition
	}
