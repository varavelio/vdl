...
func generateProcedureTypes(schema *irtypes.IrSchema, _ *config.TypeScriptConfig) (string, error) {
	...
	if len(schema.Procedures) > 0 {
		...
		for _, proc := range schema.Procedures {
			...
			g.Line(renderHydrateType("", outputName, proc.OutputFields))
			g.Break()

			g.Line(renderValidateType("", inputName, proc.InputFields))
			g.Break()
