...
func buildVDLBinary() error {
	vdlBinaryPath = filepath.Join(os.TempDir(), "vdl-ir-e2e-test")
...
func runTestCase(t *testing.T, caseDir string) {
	...
	// Find generated IR JSON file
	generatedPath := findGeneratedFile(t, caseDir)
	require.NotEmpty(t, generatedPath, "no generated IR JSON file found")
	...
	assert.Equal(t, expectedObj, generatedObj, "IR JSON mismatch")
}

// findGeneratedFile locates the generated IR JSON file in the gen directory.
